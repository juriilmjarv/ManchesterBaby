...
string compileCode(int variableNumber) 
{
	...
	//string *variableData = new string[variableNumber][4];
	//string variableData = new string[variableNumber][4];

	string** variableData = new string*[variableNumber];
	for (int o = 0; o < variableNumber; o++) {
		variableData[o] = new string[4];
	}
	variableNumber = 0;
...
int main() 
{
	...
	if (loadCodeFile(file) == 0) {
		...
		cout << "\nThe output has been written to an external file called " << newCodeFile << endl;

		string choice;

		cout << "Would you like to run the simulator? (yes/no)\n";
		cin >> choice;
		if (choice == "yes") {
			cout << " The baby simulator will now be opened...\n";
