...
int Operation::binToDecimal(string binary)
{
	...
	if (binary[0] == 1 & binary.size() > 4) {
		...
		if (location = -1) {
			...
			for (int o = location; o < tmp.size(); o++) {
				if (tmp[o] == '0') {
					tmp[o] = '1';
				}
				else {
	...
	std::cout << "string binary = " << binary << " : int value = " << value << " : binToDecimal(int binary) " << std::endl;
...
int Operation::binToDecimal(int binary)
{
	...
	std::cout << "int binary = " << binary << " : int decimalNumber = " << decimalNumber << " : binToDecimal(int binary) " << std::endl;
...
string Operation::decToBin(string dec)
{
	int value = atoi(dec.c_str()); // converts string into integer.
								   ...
								   //std::cout << "string dec = " << dec << " : decTobin(string dec) " << std::endl;
...
string Operation::decToBin(int dec)
{
	...
	if (dec < 0) {
		//2^31+(-23)
		//std::cout << "2^31 : int_pow(2, 31) " << op.int_pow(2, 31) << std::endl;

		/*
		std::cout << "int_pow " << op.int_pow(2, 31)<< std::endl;
		std::cout << "dec " << dec << std::endl;
		std::cout << "Decimal to convert " << op.int_pow(2, 31)+dec << std::endl;
		*/
		...
		*/
		int remainder, i = 1, n = dec, binNumb = 0;
		while (n != 0)
		{
			remainder = n % 2;
			n /= 2;
		...
		//binary = to_string(binNumb);
		binary = std::bitset< 31 >(binNumb).to_string(); // string conversion
	}
	//std::cout << "int dec = " << dec << " : decTobin(int dec) : Binary = " << binary << std::endl;
	return binary;
}
